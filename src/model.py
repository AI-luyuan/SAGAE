...
from .bgnn import BGNNA, BGCNA, GCNConv
...
class WBNCF(BaseModel):
    ...
    def __init__(self, n_drug, n_disease, embedding_dim=64, neighbor_embedding_dim=32, hidden_dims=(64, 32),
        ...
        self.drug_project1 = nn.Linear(n_drug, embedding_dim, bias=False)
        self.disease_project1 = nn.Linear(n_disease, embedding_dim, bias=False)



        self.self_attention_fusion1 = Self_Attention(embedding_dim,embedding_dim,embedding_dim)
        self.self_attention_fusion2 = Self_Attention(embedding_dim,embedding_dim,embedding_dim)
        self.self_attention_fusion3 = Self_Attention(embedding_dim,embedding_dim,embedding_dim)
        #self.self_attention_fusion4 = Self_Attention(embedding_dim,embedding_dim,embedding_dim)
        ...
        self.decoder = InteractionDecoder(in_channels=4*merged_dim, hidden_dims=hidden_dims, dropout=dropout,
                                          )
        self.config = config
        self.lr = lr
    ...
    def forward(self, interaction_pairs, drug_edge, disease_edge, drug_embedding, disease_embedding):
        ...
        drug_embedding1 = torch.diag(torch.ones(drug_embedding.shape[0], device=drug_embedding.device))
        disease_embedding1 = torch.diag(torch.ones(disease_embedding.shape[0], device=disease_embedding.device))

        # drug_embedding1 = self.drug_project1(drug_embedding1)
        # disease_embedding1 = self.disease_project1(disease_embedding1)

        # drug_embedding1 = F.embedding(interaction_pairs[0,:], drug_embedding1)
        # disease_embedding1 = F.embedding(interaction_pairs[1,:], disease_embedding1)




        embedding0 = torch.cat([drug_neighbor_embedding, interaction_embedding, disease_neighbor_embedding], dim=-1)
        embedding = embedding0.reshape([-1,3,self.embedding_dim])

        embedding1 = self.self_attention_fusion1(embedding)
        embedding1 = embedding1.reshape([-1,3*self.embedding_dim])

        embedding2 = self.self_attention_fusion2(embedding)
        embedding2 = embedding2.reshape([-1,3*self.embedding_dim])

        embedding3 = self.self_attention_fusion3(embedding)
        embedding3 = embedding3.reshape([-1,3*self.embedding_dim])

        #embedding4 = self.self_attention_fusion4(embedding)
        #embedding4 = embedding4.reshape([-1,3*self.embedding_dim])

        embedding = torch.cat([embedding1, embedding2, embedding3, embedding0], dim=-1)
